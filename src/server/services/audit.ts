...
export async function writeAuditLog(params: WriteAuditLogParams) {
  ...
  const log = await prisma.$transaction(async (tx) => {
    ...
    //
    // Tiebreak `id DESC` is correct because audit row ids are generated by
    // `monotonicFactory()` (see below) — within the same millisecond, the
    // ULID random component is incremented per-call, so lexicographic order
    ...
    // Defensive: treat both NULL and empty-string `tail.hash` as "no tail"
    // so that even if a historical migration default ever produced an
    // empty string, the chain anchors back to genesis rather than ''.
    const tailHash = tail?.hash && tail.hash.length > 0 ? tail.hash : null;
    const { prevHash, hash } = computeAuditChainInsert(rowToHash, tailHash);
