 ...
 * subsystem the strict-multi-tenant profile depends on is healthy. Used
 * by the load balancer's health check and by SRE dashboards. Different
 * from `/api/health/ready` (DB-only) because the strict profile has
 ...
 * server-side instead — `/api/health/deep` is intended for load-balancer
...
function logAndRedact(check: string, err: unknown): string {
  errorLog("health/deep", `${check} check failed`, err);
  return "check failed";
}
...
async function checkDatabase(): Promise<CheckResult> {
  ...
  // /api/health/deep has already returned 503.
  ...
  try {
    ...
    errorLog("health/deep", "database check failed", err);
    return {
      ok: false,
...
async function checkKms(): Promise<CheckResult> {
  ...
  try {
    ...
    if (!r.ok) {
      ...
      errorLog("health/deep", "kms healthCheck reported not-ok", r.error);
    ...
    const msg = err instanceof Error ? err.message : String(err);
    const isTimeout = /timed out|budget/i.test(msg);
    errorLog("health/deep", "kms check failed", err);
    return {
      ok: false,
 ...
 * make `/api/health/deep` permanently report 503 even on a healthy
...
async function checkClock(): Promise<CheckResult> {
  ...
  if (!r.ok) {
    ...
    errorLog("health/deep", "clock skew exceeded", { skewSeconds: r.skewSeconds, threshold: r.thresholdSeconds });
